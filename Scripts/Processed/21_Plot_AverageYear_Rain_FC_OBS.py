...
for ind_Region in range(Num_Region):
      ...
      ax.plot(np.arange(StepF_S, StepF_F+1, Disc_StepF), tp_av_year_obs_allLT[:,1], "o-", color="grey", label="OBS", linewidth=5, markersize=10)
      ...
      ax.plot(np.arange(StepF_S, StepF_F+1, Disc_StepF), y_trend, "--", color="grey", linewidth=3)
      ...
      for ind_SystemFC in range(Num_SystemFC):
            ...
            ax.plot(tp_av_year_FC[:,0], tp_av_year_FC[:,1], "o-", color=Colour_SystemFC, label=SystemFC, linewidth=5, markersize=10)

            # Creating the trend line and plotting it
            coefficients = np.polyfit(tp_av_year_FC[:,0], tp_av_year_FC[:,1], 0)
            polynomial = np.poly1d(coefficients)
            y_trend = polynomial(tp_av_year_FC[:,0])
            ax.plot(np.arange(StepF_S, StepF_F+1, Disc_StepF), y_trend, "--", color=Colour_SystemFC, linewidth=3)
