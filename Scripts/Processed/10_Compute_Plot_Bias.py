...
#####################################################################################
# CODE DESCRIPTION
# 10_Compute_Plot_Bias.py computes and plots frequency bias.
            ...
            for StepF in range(StepF_Start, (StepF_Final+1), Disc_Step):
                  ...
                  for indSystemFC in range(len(SystemFC_list)):
                        ...
                        for indRegion in range(len(RegionName_list)): 
                              ...
                              perc = pd.read_csv(DirIN_temp + "/" + FileNameIN_temp).to_numpy()[:,0]
                              bias = ( ct_tot[:,0] + ct_tot[:,1] ) / (ct_tot[:,0] + ct_tot[:,2])
                              
                              # Plotting the ROC curves
                              ax.plot(perc, bias, Lines_Region, color=Colour_SystemFC, label=SystemFC + " - " + RegionName)
                              

                  # Setting the plot metadata
                  ax.plot([0,100], [1,1], "-", color="black")
                  ...
                  ax.set_xlabel("Ensemble Member [-]", fontsize=18, labelpad=10)
                  ...
                  # Saving the plot
                  DirOUT_temp= Git_repo + "/" + DirOUT + "/" + f"{Acc:02d}" + "h/EFFCI" + f"{EFFCI:02d}" + "/VRE" + str(MagnitudeInPerc_Rain_Event_FR)
                  FileNameOUT = "ROC_" + f"{Acc:02d}" + "h _EFFCI" + f"{EFFCI:02d}" + "_VRE" + str(MagnitudeInPerc_Rain_Event_FR) + "_" + f"{StepF:03d}" + ".jpeg"
