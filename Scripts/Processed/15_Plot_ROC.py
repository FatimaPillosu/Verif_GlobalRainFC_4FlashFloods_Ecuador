...
#####################################################################################
# CODE DESCRIPTION
# 15_Plot_ROC.py plots ROC curves.
...
DirIN = "Data/Compute/12_Daily_Prob_Contingency_Tables"
DirOUT = "Data/Plot/15_ROC"
#####################################################################################

# Plotting ROC curves for a specific EFFCI index
for EFFCI in EFFCI_list:
       
       # Plotting ROC curves for a specific VRT
      for MagnitudeInPerc_Rain_Event_FR in MagnitudeInPerc_Rain_Event_FR_list:
            ...
            for StepF in range(StepF_Start, (StepF_Final+1), Disc_Step):
                  ...
                  print("Plotting the ROC curves for EFFCI>=" + str(EFFCI) + ", VRT>=tp(" + str(MagnitudeInPerc_Rain_Event_FR) + "th percentile), and StepF=" + str(StepF) + " ...") 
                  ...
                  for indSystemFC in range(len(SystemFC_list)):
                        ...
                        for indRegion in range(len(RegionName_list)): 
                              ...
                              TheDate = DateS
                              while TheDate <= DateF:
                                    DirIN_temp= Git_repo + "/" + DirIN + "/" + f"{Acc:02d}" + "h/EFFCI" + f"{EFFCI:02d}" + "/VRT" + f"{MagnitudeInPerc_Rain_Event_FR:02d}" + "/" + f"{StepF:03d}" + "/" + SystemFC + "/" + RegionName
                                    FileNameIN_temp = "CT_" + f"{Acc:02d}" + "h_EFFCI" + f"{EFFCI:02d}" + "_VRT" + f"{MagnitudeInPerc_Rain_Event_FR:02d}" + "_" + SystemFC + "_" + RegionName +"_" + TheDate.strftime("%Y%m%d") + "_" + TheDate.strftime("%H") + "_" + f"{StepF:03d}" + ".csv"
                  ...
                  ax.plot([0,1], [0,1], "-", color="grey", linewidth=3)
                  ax.set_title("ROC curve\n" + r"EFFCI>=" + str(EFFCI) + " - VRT>=tp(" + str(MagnitudeInPerc_Rain_Event_FR) + "th percentile) - StepF=" + str(StepF), fontsize=20, pad=20, color="#333333", weight="bold")
                  ax.set_xlabel("False Alarm Rate [-]", fontsize=20, labelpad=10, color="#333333")
                  ax.set_ylabel("Hit Rate [-]", fontsize=20, labelpad=10, color="#333333")
                  ax.set_xlim([0,1])
                  ax.set_ylim([0,1])
                  ...
                  ax.legend(loc="lower right",  ncol=2, fontsize=20)
                  ax.grid()
                  
                  # Saving the plot
                  DirOUT_temp= Git_repo + "/" + DirOUT + "/" + f"{Acc:02d}" + "h/EFFCI" + f"{EFFCI:02d}" + "/VRT" + f"{MagnitudeInPerc_Rain_Event_FR:02d}"
                  FileNameOUT_temp = "ROC_" + f"{Acc:02d}" + "h_EFFCI" + f"{EFFCI:02d}" + "_VRT" + f"{MagnitudeInPerc_Rain_Event_FR:02d}" + "_" + f"{StepF:03d}" + ".jpeg"
